...
(function(){
	...
	function build() {
		...
		// ajax
		//Donna - Altered this to use makeRequest in order to avoid problems with cross-domain images.
		...
		svg.AspectRatio = function(ctx, aspectRatio, width, desiredWidth, height, desiredHeight, minX, minY, refX, refY) {
			...
			// calculate scale
			var scaleX = width / dWidth;
			var scaleY = height / dHeight;
			var scaleMin = Math.min(scaleX, scaleY);
			var scaleMax = Math.max(scaleX, scaleY);
			if (meetOrSlice == 'meet') { dWidth *= scaleMin; dHeight *= scaleMin; }
			if (meetOrSlice == 'slice') { dWidth *= scaleMax; dHeight *= scaleMax; }	
			...
			if (align == 'none') ctx.scale(scaleX, scaleY);
				...
				//Donna - Don't scale the image larger than its current size.
				if (scaleMin > 1) {
					ctx.scale(1, 1);
				}
				else {
