 ...
 * 1.0.1
 *   - Fixed bug with interactive elements and made scrolling smoother (thanks Paul and
 *   Aktar)
 */
...
(function(w, m, $, o){
	...
	$.extend(o, {
		...
		init: function(target, options, data) {
			...
			data = {
				sizing: o.getSizing(target, options)
			};
			...
			// set css
			options.openedCss = {cursor: "default"};
			options.closedCss = {cursor: "default"};
			...
			if(options.showThumbs) {
				...
				if(data.sizing.container.scrollWidth > 0 && options.direction !== 'vertical') {
					data.thumbs.horizontal = $(o.div).css(o.getThumbCss(data.sizing.thumbs.horizontal, options.thumbsColor));//.fadeTo(0, 0);
					target.prepend(data.thumbs.horizontal);	
				}
				
				if(data.sizing.container.scrollHeight > 0 && options.direction !== 'horizontal') {
					data.thumbs.vertical = $(o.div).css(o.getThumbCss(data.sizing.thumbs.vertical, options.thumbsColor));//.fadeTo(0, 0);
		...
		toggleDragMode: function(data, dragging) {
			...
			data.thumbs.vertical.fadeTo("fast", o.constants.thumbOpacity);
                    }
                    if(data.thumbs.horizontal) {
                        //data.thumbs.horizontal.fadeTo("fast", 0);
			data.thumbs.horizontal.fadeTo("fast", o.constants.thumbOpacity);
                    }
                }
		    }
		},
		...
		getSizing: function(container, options, sizing) {
			...
			sizing.thumbs = {
				horizontal: {
					...
					height: options.thumbThickness,
					corner: options.thumbThickness / 2,
					//height: o.constants.thumbThickness,
					//corner: o.constants.thumbThickness / 2,
					left: 0,
					top: sizing.container.height - options.thumbThickness
					//top: sizing.container.height - o.constants.thumbThickness
				},
				vertical: {
					width: options.thumbThickness,
					//width: o.constants.thumbThickness,
					//height: sizing.container.height * sizing.container.height / sizing.container.scrollHeight,
					height: sizing.container.height * sizing.container.height / container[0].scrollHeight,
					corner: options.thumbThickness / 2,
					left: sizing.container.width - options.thumbThickness,
					//corner: o.constants.thumbThickness / 2,
		...
		getThumbCss: function(size, thumbsColor) {
		
			return {
				position: "absolute",
				"background-color": thumbsColor,
				width: size.width + "px",
