...
(function($, window, document) {
	...
	$.fn.dataTable = function( oInit )
	{
				...
				settings.jqXHR = $.ajax( {
					...
					"error": function (xhr, error, thrown) {
						if ( error == "parsererror" ) {
							console.log( "DataTables warning: JSON data from server could not be parsed. "+
		...
		function _fnFeatureHtmlTable ( oSettings )
		{
			...
			{
				//nScrollHead.style.overflow = "hidden";	//Donna
				nScrollHead.style.position = "relative";
				nScrollFoot.style.overflow = "hidden";
				nScrollBody.style.overflow = "hidden";	//Donna
			}
		...
		function _fnScrollDraw ( o )
		{
			...
			_fnApplyToChildren( function(nSizer, nToSize) {
				...
				//Donna
				//iWidth = $(nSizer).width();
				iWidth = $(nSizer).outerWidth();
				nToSize.style.width = _fnStringToCss( iWidth );
				
				aApplied.push( iWidth );
			}, anHeadSizers, anHeadToSize );
			$(anHeadSizers).height(0);
			$(anHeadSizers).css("visibility", "hidden");
		...
		function _fnSortingClasses( oSettings )
		{
			...
			var aaSort, sClass, sArrow, sIndicator;
			var iColumns = oSettings.aoColumns.length;
			var oClasses = oSettings.oClasses;
			...
			for ( i=0 ; i<iColumns ; i++ )
			{
				if ( oSettings.aoColumns[i].bSortable )
				{
					...
					$(oSettings.aoColumns[i].nTh).find(".arrow").html("");		//Donna
					$(oSettings.aoColumns[i].nTh).find(".indicator1").html("");	//Donna
					$(oSettings.aoColumns[i].nTh).find(".indicator2").html("");	//Donna
				}
			}
					...
					for ( j=0 ; j<aaSort.length ; j++ )
					{
						if ( aaSort[j][0] == i )
						{
							...
							if (sClass == oClasses.sSortAsc) {
								$(oSettings.aoColumns[i].nTh).find(".arrow").html("&darr;");
								$(oSettings.aoColumns[i].nTh).find(".indicator1").html("A");
								$(oSettings.aoColumns[i].nTh).find(".indicator2").html("Z");
							}
							else {
								$(oSettings.aoColumns[i].nTh).find(".arrow").html("&uarr;");
								$(oSettings.aoColumns[i].nTh).find(".indicator1").html("Z");
								$(oSettings.aoColumns[i].nTh).find(".indicator2").html("A");
							}
		...
		function _fnCalculateColumnWidths ( oSettings )
		{
			...
			else
			{
				...
				//_fnScrollingWidthAdjust( oSettings, nCalcTmp );	//Donna - No scrollbar is shown in Gadgets that use this plugin.
				...
				for ( i=0 ; i<oSettings.aoColumns.length ; i++ )
				{
					if ( oSettings.aoColumns[i].bVisible )
					{
						...
						iWidth = $(oNodes[iCorrector]).outerWidth();	//Donna - Responsive design
