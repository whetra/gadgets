...
Graph.prototype = {
  ...
  _initCanvas: function(){
    ...
    for (i = children.length; i--;) {
      ...
      //Issue 929 Start - Get rid of overlay
    ...
    //this.overlay = getCanvas(this.overlay, 'overlay');  //Issue 929
    this.ctx = getContext(this.canvas);
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    //this.octx = getContext(this.overlay); //Issue 929
    //this.octx.clearRect(0, 0, this.overlay.width, this.overlay.height); //Issue 929
	...
	canvas.style.cssText = 'position:absolute;left:0px;'; //Donna
        //canvas.style.cssText = 'position:absolute;left:0px;top:0px;';
        D.insert(el, canvas);
        
        //Issue 927 Start - Moved this inside if statement.
...
Axis.prototype = {
  ...
  _calculateTicks : function () {
    ...
    for (i = 0; (v = v2 = start + i * tickSize) <= max; ++i){
      ...
      //Donna Start - Only add tick if it has not already been added.
      for (j = 0; j < axis.ticks.length; ++j) {
        if (v == axis.ticks[j].v) {
            found = true;
            break;
        }
      ...
      if (axis.options.showLabels && (isFirst ? true : axis.used)) {
        for (i = 0; i < axis.ticks.length; ++i) {
          ...
          html += [
            ...
            'top:' + top + 'px; ',
            ((!isX) ? 'right' : 'left:') + left + 'px; ', //Donna
            //((!isX && !isFirst) ? 'right:' : 'left:') + left + 'px; ',
            (isX ? 'width:' + xBoxWidth + 'px; ' : ((isFirst ? offset.left : offset.right) - options.grid.labelMargin)),  //Donna
...
Flotr.addType('lite-lines', {
  options: {
    ...
    fillBorder: false,     // => draw a border around the fill
    fillColor: null,       // => fill color
    lineStyle: "solid"     // => style of line, either 'solid' or 'dashed' - Donna
  },
  ...
  draw : function (options) {
    ...
    //Donna Start - Add support for dashed lines.
    if (options.lineStyle == "solid") {
        this.plot(options);
    }
    else if (options.lineStyle == "dashed") {
        this.dashedLineTo(options, [10,10]);
    }
    //Donna End
  ...
  //Donna - Add support for dashed lines.
  dashedLineTo : function (options, pattern) {
      var lt = function (a, b) { return a <= b; },
          ...
          fromX = xScale(data[0][0]);
          fromY = yScale(data[0][1]);
          toX = xScale(data[length][0]);
          toY = yScale(data[length][1]);
  ...
  plot : function (options) {
    ...
    if (options.fill){
      ...
      context.fill();
      if (options.fillBorder) {
        //Donna Start - Issue 909: Reset path.
        x0 = xScale(data[0][0]);
